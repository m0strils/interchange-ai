...
python interchange.py --profile <yours> --ask "…"   # any profile; --context notes|chunks --budget-chars N --note-max-chars N override the profile (ADR-0018)
make eval PROFILE=<yours> K=4 CONTEXT=notes         # passage@k, context@k, note@k, chars on the assembled context (ADR-0017/0018)
# any profile, including ones defined only in the private overlay (~/.interchange/profiles.yaml, ADR-0016)
python interchange.py --profile <yours> --golden-add --question "…" --expected "path/in/corpus.md"   # eval-as-you-go: append a validated golden row
scripts/launchd/install.sh <yours>                  # nightly 03:30 full reindex for one profile (read-only, $0); measured 30 s for the 4,614-chunk vault (ADR-0016)
...
**Status:** ADR-0018 shipped — **context assembly with a budget**: the retrieval unit
stays the chunk, the context unit becomes the note; fair-share expansion after ranking,
measured (brain context@4 2/7 → 7/7, graded 40% → 93%, zero ranking changes), HTTP
`context` knob locked by default, judge grades the governed path. ADR-0017 **kept the
passage metric and structural lead handling but rejected the ingest merge on
measurement** (carried by ADR-0018). ADR-0016 made **corpus profiles portable data**: a
private overlay (`INTERCHANGE_PROFILES`) adds or extends profiles outside the tree,
`INTERCHANGE_CHROMA_DIR` relocates the store, `--profile` applies a profile in-process
(retiring the export-before-`.env` edge), and `persona` / `retrieval` / `tools` are
profile keys resolved per request by collection. Personal corpora — a second-brain
Obsidian vault, a `research` corpus over saved research dumps — are overlay-only
profiles; nothing personal enters the repo. Rebuild times are **measured** (vault 4,614
chunks / 30 s; brain 407 chunks / 3 s, ADR-0016); incremental reindex stays
trigger-gated. ADR-0015 shipped a same-origin **browser workbench** at `/ui` over the
...
ADR-0014 shipped the vault corpus (188 notes, 4,614 chunks) ingested read-only with
**measured** retrieval ablations (`eval/eval-runs.jsonl`): ADR-0007's reranker trigger
fired and paid off (hit@1 11→15/18, local cross-encoder, $0), while naive link-aware
expansion regressed and stays deferred.
