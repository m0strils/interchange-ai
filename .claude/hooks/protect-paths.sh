...
# run must not change (its own acceptance gate, the golden eval set). Exit 2 blocks
...
PROTECTED = ("scripts/a2a-accept.sh", "scripts/workbench-accept.sh", "scripts/workbench_accept.py", "eval/golden.jsonl", ".claude/verify-gate", ".claude/hooks/", ".claude/settings.json")
